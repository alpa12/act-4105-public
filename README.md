...
- Pour exporter toutes les présentations de chapitres depuis la racine du dépôt, lancer :

  ```bash
  scripts/render-diapos-pdfs
  ```

  Le script découvre tout dossier numérique contenant un `diapos.qmd`, ignore le chapitre `00`, et écrit un PDF par chapitre dans `site/pdfs/diapos/`. Les PDF d’exercices corrigés se génèrent séparément avec `scripts/render-exercise-pdfs` : il rend chaque `exercices.qmd` numérique (sauf `00`) au format `exam-typst` de l’extension `alpa12/exam`, avec la couverture et les instructions communes définies dans `site/instructions-exercices.qmd`. Les solutions et les blocs `.grading` présents sont affichés, puis seuls les PDF sont écrits dans `site/pdfs/exercices/`. Aucun bloc `.grading` n’est ajouté aux sources. Ces dossiers sont volontairement ignorés par Git.
...
Le script rend la source, résout les actifs depuis `site/_site`, sert le HTML localement, prépare `print-pdf` et imprime via CDP. `scripts/render-diapos-pdfs` exporte tous les chapitres numériques dans le répertoire ignoré `site/pdfs/diapos/`, avec un sommaire PDF navigable composé des titres H1 et H2 (les répétitions H2 consécutives sont regroupées). Lorsqu’un titre est coupé visuellement, l’export ajoute un séparateur de ligne invisible dans les métadonnées pour que Chrome conserve les espaces du signet. Les lignes de la diapositive imprimée « Contenu du chapitre » sont aussi des liens internes vers leurs sections H1. `scripts/revealjs-pdf` conserve son comportement par défaut; son option interne `--outline-h1-h2` active ce sommaire lorsque nécessaire. Vérifier taille, nombre de pages, dimensions paysage, pages-titres, vues d’ensemble, tableaux, formules, images, diapositives denses, liens du plan et signets PDF.
