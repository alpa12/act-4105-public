...
- Batch PDF exporters: `scripts/render-diapos-pdfs` writes one PDF for every numeric chapter presentation except `00` to the Git-ignored `site/pdfs/diapos/` directory. It discovers future numeric chapter directories automatically, delegates each export to `scripts/revealjs-pdf`, and enables an H1/H2 PDF navigation outline (consecutive duplicate H2 entries are grouped). `scripts/render-exercise-pdfs` writes a `exam-typst` PDF for every numeric `exercices.qmd` except `00` to `site/pdfs/exercices/`; the `exercices-pdf` profile activates `alpa12/exam`, sets its common course metadata, enables `show-solutions` and `show-grading`, and uses `site/instructions-exercices.qmd` for the common cover instructions. The script retains only the PDF output. The presentation export inserts invisible soft breaks in printed headings so Chrome retains spaces in outline labels when a title wraps. The rows in the printed chapter overview are internal PDF links to their H1 sections. Direct `scripts/revealjs-pdf` calls remain outline-free unless they pass `--outline-h1-h2`.
