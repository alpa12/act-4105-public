...
library(tarifr)
here::i_am("site/chapitres/06-sinistres/diapos.qmd")
source(here::here("site/_extensions/alpa12/dynamic-year/dynamic-year.R"))
```
...
Les **sinistres payés** durant une année de calendrier considèrent tous les sinistres payés dans l’année de calendrier, sans égard à la date d’accident ou la date rapportée.
...
Au 31 décembre de l’année d’accident, les sinistres rapportés vont souvent changer lorsque des réclamations additionnelles sont rapportées, lorsque des réclamations sont payées ou lorsque les réserves changent.
...
Au 31 décembre de l’année de police, les sinistres vont souvent changer car d’autres réclamations additionnelles surviennent, des réclamations sont payées ou les réserves changent.
...
Même si les transactions de sinistres ont lieu après l’année de police en question.
...
Méthode utilisée typiquement pour les produits des lignes commerciales avec polices sur base de réclamations présentées _(ang. : claims-made policies)_.

---

Sinistres **rapportées** du 1er janvier au 31 décembre.
...
- Ces transactions proviennent de polices émises avant l’année d’accident en question
...
Afin de balancer l’équation fondamentale en assurance, on doit tout de même inclure une provision pour la portion des pertes majeures qui est au-dessus du seuil.

On regardera donc ces sinistres sur une plus grande période. On assume que, quoique très volatile à court terme, la proportion des sinistres attribuable aux pertes majeures sera assez stable à long terme.
...
afficher_tableau_sinistres_excedentaires()
```
...
Tel que discuté avec les sinistres individuels très élevés, on calcule le ratio des sinistres pour ces catastrophes divisés par les sinistres non catastrophiques sur une longue période.
...
Des modèles de catastrophes existent pour prédire les dommages résultants.
...
Sinistres rapportés (en milliers de dollars), par âge (en mois) d’année d’accident, vues au {{< dynamic-date "YYYY-03-31" -1 >}}.
...
- Régression linéaire
  - On calcule le changement en additif d’une période à l’autre
...
- Année de {{< dynamic-year -4 >}}, police de 1 an
...
Attention à l’effet des limites versus la tendance dans la sévérité.
...
- Non alloués : méthode simple d’allocation des frais de règlement de sinistres non alloués
  - Hypothèses de base :
    - Les frais de règlement de sinistres non alloués suivent les montants de sinistres et frais de règlement de sinistres alloués de façon consistante d’une année à l’autre
  ...
  - On calcule le % frais de règlement de sinistres non alloués sur quelques années de calendrier par rapport aux montants de sinistres et frais de règlement de sinistres alloués


:::: {.question title="Frais de règlement non alloués"}
