...
**Transactions** ayant lieu du 1er janvier au 31 décembre. Les quantités sont fixes à la fin de l'année.
...
**Accidents** ayant lieu du 1er janvier au 31 décembre, sans égard au moment où la police a été écrite ou au moment où le sinistre est rapporté.
...
On considère les sinistres associés aux **polices émises** du 1er janvier au 31 décembre.
...
# Ajustements aux sinistres {data-progress-label="Ajustements" subtitle="Sinistres extraordinaires, changements à la couverture, développement et tendance"}
...
afficher_tableau_sinistres_excedentaires(1:7)
```
...
- Aire de la section 2 : $1.5 / 3 \times (1.5 / 12) / 2 = 0.03125$
...
tableau_ultimes <- data.frame(
  ...
  "Facteur de dév. à l'ultime" = facteurs_ultimes,
  check.names = FALSE
)
...
::: {.column width="40%"}
*L'__âge__ des sinistres dans ce tableau (15, 27, 39...) correspond au nombre de mois écoulés depuis le premier janvier de la date d'accident.*
:::
::::
...
::: {.column width="45%"}
```{r}
#| label: chain-ladder-triangle-table-copy
#| echo: false
#| output: asis
...
Exemples de calcul pour l'AC {{< dynamic-year -7 >}} :
$$
1.50 = 1500 / 1000 \qquad 1.28 = 1925 / 1500
$$

##

### Sinistres rapportés développés à l'ultime {.text-small}
...
Le facteur de développement à l’ultime est le produit de tous les facteurs de développement sélectionnés à partir de l'âge des sinistres.
Pour l’AC {{< dynamic-year -2 >}}, âgé de 15 mois, le calcul est donc : $1.50 \times 1.29 \times 1.10 \times 1.02 \times 1 = 2.17$.
...
Ajustements possibles aux sinistres :

- Sinistres extraordinaires (pertes majeures et catastrophes)
- Changements à la couverture
- Développement des sinistres
- Tendance dans les sinistres
...
sinistres_severite <- data.frame(
  "No. réclamation" = 1:5,
  "Sinistre non limité ($)" = c(10000, 15000, 24000, 30000, 50000),
  check.names = FALSE
)
```
