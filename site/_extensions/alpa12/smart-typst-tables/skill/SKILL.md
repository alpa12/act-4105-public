...
No option is required for a Pandoc header containing `colspan` or `rowspan`.
HTML and Revealjs add `smart-table-has-grouped-headers` to the table,
`smart-table-header-group` to merged cells, and the stable `-start` / `-end`
boundary classes to the physical header cells. Merged labels are centered even
when their source column is right-aligned. Eligible Typst tables preserve the
same header spans natively; body and footer spans remain a conservative
fallback.

To theme only the grouped-header treatment, use the public CSS variables
`--smart-table-grouped-header-fill`, `--smart-table-group-boundary-color`, and
`--smart-table-group-boundary-width`; see the HTML/Revealjs reference for an
override example.
