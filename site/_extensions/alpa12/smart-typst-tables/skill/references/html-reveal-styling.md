...
- `.smart-table-has-grouped-headers`: marks a table with a merged Pandoc
  header. Its merged cells receive `.smart-table-header-group`; cells that
  continue a group's left/right edge receive `-start` / `-end` (and may have
  both).
...
For grouped headers, override these public root/theme variables without
targeting the generated table structure:

```scss
:root {
  --smart-table-grouped-header-fill: #e8eef3;
  --smart-table-group-boundary-color: #5f6d78;
  --smart-table-group-boundary-width: 1px;
}
```

Grouped-header labels are centered deliberately, even when Pandoc supplied an
inline right alignment. The variables also include `--smart-table-border`, `--smart-table-radius`,
