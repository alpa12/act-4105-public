...
Typst supports simple header-only `colspan` and `rowspan` cells. It serializes
their physical source rows with native `table.cell` spans, centers merged
labels, and draws the same grouped-header boundaries with `table.vline`.

Typst leaves a table unchanged when it has body/footer spans, rich inline content (links,
