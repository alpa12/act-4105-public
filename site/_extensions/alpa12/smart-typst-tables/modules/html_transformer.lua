...
local function text_inlines(text)
  local inlines = {}
  local first = true
  for _, word in ipairs(metrics.split_words(text)) do
    if not first then
...
local function transform_header(tbl, model, plan, options, target)
  ...
  local geometry = model.header_geometry or table_ast.header_geometry(model)
  ...
  for row_index, row in ipairs(tbl.head.rows) do
    ...
    if row_role then
      row.attr = add_class(row.attr or pandoc.Attr(), "smart-table-header-row-" .. row_role)
    end
    for cell_index, cell in ipairs(row.cells) do
      ...
      -- Calculation references are header content, but never layout evidence.
      -- They receive the column plan inferred from `labels`, while their
      -- source inlines remain untouched below.
      local is_calculations = row_role == "calculations"
      ...
      if col_span ~= 1 or row_span ~= 1 then
        table.insert(classes, "smart-table-header-group")
      end
      local role = geometry.roles[row_index] and geometry.roles[row_index][cell_index]
      if role and role.start then
        table.insert(classes, "smart-table-header-group-start")
      end
      if role and role.finish then
        table.insert(classes, "smart-table-header-group-end")
      end
      local width = nil
      if target == "revealjs" and plan.table_width == "natural"
        and logical and col_span == 1 then
        width = reveal_track_width(model, plan, col)
      end
      style_cell(cell, classes, col_span > 1 and "colgroup" or "col", width)
      if is_calculations and logical then
        -- Pandoc turns a leading `(1)` in a grid-table cell into an ordered
        -- list. Rehydrate only the calculation notation, never the labels or
        -- a data row, so the browser receives the formula written by the
        -- author rather than an accidental list.
        local calculation = table_ast.calculation_text(logical.cell)
        if calculation then
          cell.contents = { pandoc.Plain(text_inlines(calculation)) }
        end
      end
      if not is_calculations and logical and col_span == 1 and is_simple_text_blocks(cell.contents) then
        ...
        if row_role ~= "labels" and (row_index > 1 or not lines) then
...
function M.render(tbl, model, plan, options, target)
  ...
  if model.header_geometry and model.header_geometry.has_grouped_headers then
    add_class(tbl.attr, "smart-table-has-grouped-headers")
  end
