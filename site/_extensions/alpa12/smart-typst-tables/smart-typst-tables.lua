...
local function table_header_signature(tbl)
...
local function header_role_table_signatures(meta)
  local value = meta and meta["smart-tables-header-role-signatures"]
  local signatures = {}
  for _, entry in ipairs(value or {}) do
    local signature = pandoc.utils.stringify(entry)
    signatures[signature] = (signatures[signature] or 0) + 1
  end
  return signatures
end

local function restore_header_roles(tbl)
  tbl.attr = tbl.attr or pandoc.Attr()
  tbl.attr.attributes = tbl.attr.attributes or {}
  tbl.attr.attributes["smart-tables-header-roles"] = "labels,calculations"
end
...
local function render_table(tbl, attr)
  ...
  if attr then
    ...
    table_ast.set_header_roles(model)
    table_ast.refresh_complex_content(model)
...
local function table_filter(tbl)
  local signature = table_header_signature(tbl)
  if (state.header_role_table_signatures[signature] or 0) > 0 then
    restore_header_roles(tbl)
    state.header_role_table_signatures[signature] = state.header_role_table_signatures[signature] - 1
  end
  return render_table(tbl)
end

local function initialize_filter(doc)
  if not target_format() then
    return doc
  end

  state.options = config.from_meta(doc.meta)
  state.header_role_table_signatures = header_role_table_signatures(doc.meta)
  return doc
end

return {
  { Pandoc = initialize_filter },
  { Div = div_filter },
  { Table = table_filter },
}
